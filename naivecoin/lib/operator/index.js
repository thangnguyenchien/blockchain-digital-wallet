...
class Operator {
    ...
    isTransactionInBlockchain(transactionId) {
        let transaction = this.blockchain.getTransactionFromBlocks(transactionId)
        if(!transaction) {
            throw new ArgumentError(`Transaction id ${transactionId} is not in blockchain`)
        }
        return transaction
    }
